...
with st.sidebar: 
    ...
    try:
        expiration_dates = [date for date in stock.options if datetime.strptime(date, "%Y-%m-%d") > today + timedelta(days=7)]
    except:
        expiration_dates = []
    ...
    min_input = st.number_input("Minimum Strike Price (as % of Spot Price)", value=0.8, min_value =0.5)  
...
def fetch_option_data(ticker, risk_free_rate):
    ...
    for exp_date in expiration_dates:
        ...
        time_to_maturity_years = (expiration_datetime - today).days / 252.0  
